...
static void PTP_LogSampleIfReady(void)
{
    ...
    len = snprintf(
        ptp_log_buf,
        sizeof(ptp_log_buf),
        "DATA,%lu,%ld,%ld,%ld,%ld,%lu,%ld,%lu,%lu,%lu,%lu\r\n",
...
int main(void)
{
  ...
  const char *hdr =
      "FMT,sample,offset_ns,offset_avg_ns,mean_path_delay_ns,freq_err_ppb,current_addend,last_addend_step,phase_step_count,freq_update_count,sync_rx_ts_valid_count,tx_ts_seen\r\n";
