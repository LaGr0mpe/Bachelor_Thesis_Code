...
#define PTP_ANNOUNCE    11U

#define ETH_RX_BUFFER_SIZE            1524U
#define PTP_WAIT_LOOP                 1000000UL

/* PTP system-time generation configuration. */
...
volatile uint32_t delayreq_tx_desc_idx = 0U;
...
volatile uint32_t ptp_rx_overwrite_count = 0U;
volatile uint32_t ptp_follow_overwrite_count = 0U;
volatile uint32_t ptp_delayresp_overwrite_count = 0U;
...
static void PTP_HW_Init(void)
{
    ...
    /* Accept all multicast and PTP Ethernet frames. */
    ETH->MACFFR |= ETH_MACFFR_RA | ETH_MACFFR_PM;

    /* Mask timestamp trigger interrupt: target-time interrupt is not used yet. */
    ETH->MACIMR |= (1UL << 9);

    /* Keep the currently working digital-rollover timestamp format. */
...
static void PTP_BuildDelayReqFrame(uint8_t *frame, uint16_t seq_id)
{
    ...
    /* correctionField = 0, bytes 22..29 already zero */
...
static void PTP_SendDelayReq(void)
{
    ...
    /* Request hardware TX timestamp for this frame. */
    td[0] |= (1UL << 25);
...
static void PTP_Process(void)
{
    ...
    if (g_follow_evt.pending)
    {
        ...
        if ((g_follow_evt.seq_id == sync_seq_id) && (sync_received != 0U))
        {
            ...
            {
                check_1++;
                if (PTP_ApplyTimeInitialize(follow_sec, follow_nsec))
                {
                    ptp_synced = 1U;
                }
            }
